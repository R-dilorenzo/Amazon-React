...
function Home() {
    const [{ prodotti, categoria, ratingRichiesto,minPrezzo,maxPrezzo,type }, dispatch] = useStateValue();
    ...
    let allFalse = Object.keys(type).every(function(valItem){ return type[valItem] === false });
        ...
        <div className="home">
            ...
            <div className="filter">
                <div className="filter__component">
                    <strong className="filter__subtitle">
                        Cerca per categoria/e:
                    </strong>
                    {allFalse ? (
                        //condition true
                        <div>
                    ...
                    {ratingRichiesto >1  ? (
                        //condition true
                        <strong>
                            Attualmente è impostato il filtro per {ratingRichiesto}<StarIcon className="filter__iconSearch" ></StarIcon> o più
                        </strong>
                    ) : (
                        //condition false
                        <div> </div>
                    ...
                    <div  className="filter__priceF">
                        <div className="tr">
                            ...
                            <div className="td">
                                <input 
                                type="text" id="MinPrice"  className="filter__inputPrice"
                                placeholder={minPrezzo}
                                onChange={changePriceMin}
                                onKeyDown={ (evt) => !(evt.key >= '0' && evt.key <= '9' || evt.key === '.' || evt.keyCode === 8) && evt.preventDefault() }
                                />                            
                            </div>
                        </div>
                        <div className="tr">
                    ...
                    <div className="row">
                        {
                            (arrayProdotti = renderProdotti()
                                .filter((filterRatings) => 
                                    ...
                                    .filter((filterPrezzo) =>
                                        filterPrezzo.price >= minPrezzo && filterPrezzo.price <= maxPrezzo
                                        )
                                        .map((item) => (
